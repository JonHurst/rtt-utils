...
def process_doubles(p):
    ...
    #double preceded by space and followed by word character is an opening double
    p = re.sub(r'(\s+)"([\w' + opening_microformat_char + '])', r'\1“\2', p)
    #double preceded by a character that is neither word character nor
    #space and followed by space is a closing double
...
def process_singles(p):
    ...
    #space and followed by space is a closing single
    p = re.sub(r"([^\s\w])'(\s+)", r'\1’\2', p)
